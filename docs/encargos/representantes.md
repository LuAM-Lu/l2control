Construye «Personas → Representantes y niños»: el directorio mínimo de familias del parque. Es la tarea 4.5 del plan final (docs/PLAN-FRONTEND.md) y cubre F5-01, con la decisión DEC-9.

QUÉ ES Y QUÉ NO ES: DEC-9 fijó **lo menos sensible que permite operar**: nombre, apodo opcional, edad opcional y una referencia de contacto para una urgencia. Sin documento de identidad, sin dirección, sin foto. El contrato no admite nada más, y la pantalla tampoco debe pedirlo.
...
- `GuardianSchema` y `KidSchema` ya existían y siguen siendo los de la entrada.
...
NO HAGAS:
- No toques `packages/`, ni la entrada, ni la salida, ni el monitor.
- No añadas campos que DEC-9 no autorizó, ni un botón de borrar.
- No inventes visitas editables: son un dato del servidor.

CRITERIO DE TERMINADO: "pnpm typecheck" pasa sin errores (ejecútalo tú desde la raíz de la copia y lee su salida). Resume qué hiciste en cada punto. La maestra probará: buscar por teléfono sin guiones, corregir un apodo, y dar a una familia el teléfono de otra, que el contrato debe rechazar con su mensaje.
