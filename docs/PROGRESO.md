...
| F6-03 Carta táctil y editor | Parcial | Categorías y platos de 88 px, agotados visibles pero no pedibles, borrador con notas y confirmación antes de cocina. **Editor en Panel → Restaurante → Carta y precios** (2026-09-16, primer encargo de la orquesta): borrador con deshacer y «Publicar», alta y edición en hoja, agotar y reponer, **retirar sin borrar** (`retiredAt`); el contrato rechaza precios en cero, nombres repetidos en venta y cartas sin nada que vender (4 pruebas). El salón lee la carta publicada. **Carta y precios inventados hasta F0-04**; faltan modificadores (F6-04) y tipo de IVA por plato |
...
| F5-04 Paquetes de tarifa | Parcial | Selector con botones grandes sobre el catálogo del contrato. **Contrato del tarifario listo** (2026-09-16, `TarifarioSchema`, 9 pruebas): paquetes y política se validan juntos —al menos uno a la venta, precios en dólares y mayores que cero, sin nombres repetidos, pase libre solo al salir, aviso menor que el paquete más corto—. El editor está encargado a la obrera ([encargo](encargos/tarifas-editor.md)) |
...
| F2-13 Roles y accesos de la sucursal | Parcial | **Alcance añadido el 2026-09-14.** Panel → Configuración → Roles y accesos: la matriz de §7.3 como base y los ajustes de este local encima, con motivo, autor y hora, retirables. Resuelve N-05: **una sola puerta al back-office** (`reportes.verSucursal`) y quién la cruza se edita aquí en vez de estar clavado. Suelo intocable comprobado en el dominio (9 pruebas): la fila de administración y las dos llaves de la casa. Comprobado en navegador: abrirle el panel a Caja deja entrar a la cajera, **sin mover su puesto de trabajo**, y sigue sin poder abrir esta pantalla |
| F2-11 Usuarios y permisos | Parcial | `/panel/personas/usuarios`: lista densa con buscador y filtro por rol, permisos plegados con su resumen, excepciones en hoja aparte y los **cinco cambios reales** —alta, baja, reingreso, rol y reponer PIN—, cada uno con motivo obligatorio y su asiento en la historia de la persona. `revisarCambio` en `@l2/domain-identity` (11 pruebas) impone las cinco puertas; `UserChangeSchema` y `UserCommandSchema` en el contrato (6 pruebas). Falta el servidor: hoy nada persiste |
| F9-08 Panel en vivo del local | Parcial (interfaz) | **Dentro de Inicio** (`/panel`), no en pantalla aparte: nació como `/panel/vivo` y duró un día, porque Inicio ya enseñaba sala, mesas y cocina con cifras escritas a mano. Cinco zonas —parque, cocina, mesas, caja y **personas conectadas** (D7)— calculadas en `vivo.ts`, puro, con los mismos eventos que mueven las estaciones. Franja de avisos que reparte el ancho de su fila, con lo crítico primero, cada aviso enlazado a donde se resuelve y movimiento por gravedad (lo crítico late; la advertencia entra y se queda quieta). **Nada se recarga**: las cuentas viajan entre pestañas por `BroadcastChannel`. Comprobado a 1366, 1280, 1024 y 420 px y con dos pestañas. Falta el tiempo real del servidor (F5-08, ADR-008) |
